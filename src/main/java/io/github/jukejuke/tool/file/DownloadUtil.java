...
import io.github.jukejuke.tool.http.HttpUtil;
...
public class DownloadUtil {
    ...
    public static String download(String urlStr, String saveDir, String fileName) {
        ...
        if (!FileUtils.mkdirs(saveDir)) {
            log.error("Failed to create save directory: {}", saveDir);
            return null;
        }
        ...
        try {
            ...
            try {
                ...
                log.error("Error closing resources during download", e);
            }
        }
    }
    ...
    private static String extractFileNameFromUrl(String urlStr) {
        try {
            ...
            log.error("Failed to extract file name from URL: {}", urlStr, e);
     ...
     * @return 下载的文件路径，如果下载失败返回null
     */
    public static String downloadToTempDir(String urlStr) {
        String tempDir = System.getProperty("java.io.tmpdir");
        return download(urlStr, tempDir);
    }
     ...
     * @return 下载的文件路径，如果下载失败返回null
     */
    public static String downloadToUserDownloads(String urlStr) {
        String userHome = System.getProperty("user.home");
        String downloadsDir = userHome + File.separator + "Downloads";
        return download(urlStr, downloadsDir);
    }
    ...
    public static byte[] downloadAsByteArray(String urlStr) {
        ...
        try {
            ...
            log.info("File downloaded successfully as byte array. Size: {} bytes", result.length);
            return result;

        } catch (Exception e) {
            log.error("Failed to download file as byte array from URL: {}", urlStr, e);
            return null;
        } finally {
            // 关闭资源
            try {
